...
import com.spring.jwt.models.*;
import com.spring.jwt.*;
import com.spring.jwt.*;
...
public class AuthController {
  ...
  public ResponseEntity<?> logoutUser(HttpServletRequest request) {
      // Extraction du token (optionnel, selon votre besoin)
      String authHeader = request.getHeader("Authorization");
      if (authHeader != null && authHeader.startsWith("Bearer ")) {
          String token = authHeader.substring(7); // Récupère le token sans "Bearer "
          // Logique optionnelle : invalider ou stocker le token dans une "blacklist"
          System.out.println("Token reçu pour déconnexion : " + token);
      }

      // Retourne une confirmation de déconnexion
